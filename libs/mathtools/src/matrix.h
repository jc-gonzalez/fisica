...
public:
    ...
    double & operator()(const int row, const int col) { return M[ row * 4 + col ]; }
    const double & operator()(const int row, const int col) const { return M[row * 4 + col]; }
    ...
    matrix4& setScale(const double scale ) { return setScale(vector3d(scale,scale,scale)); }
