...
def upckFlt(fh, nwords, bytes):
    return (bytes - nwords * wordSize,
            unpack('{}f'.format(nwords), fh.read(nwords * wordSize)))
...
def processCerFile(k, fb, newcorex=None, newcorey=None, sz=0):
    ...
    #---- Read Event Header
    bytesInBlock = unpack('1I', fb.read(1 * wordSize))[0]
    bytesInBlock = bytesInBlock - wordSize
    bytesInBlock, runh = upckFlt(fb, evthSize, bytesInBlock)
    bytesInBlock, evth = upckFlt(fb, evthSize, bytesInBlock)
    #print(evth)
    ...
    while True:
        bytesInBlock, cphotonData = upckFlt(fb, cphotonSize, bytesInBlock)
        if bytesInBlock <= wordSize:
            bytesInBlock = unpack('1I', fb.read(1 * wordSize))[0]
            bytesInBlock = unpack('1I', fb.read(1 * wordSize))[0]
            bytesInBlock = bytesInBlock - wordSize

        if pack('1f', cphotonData[0]) == 'EVTE':
            bytesInBlock, evte = upckFlt(fb, evthSize - cphotonSize, bytesInBlock)
            bytesInBlock, evth = upckFlt(fb, evthSize, bytesInBlock)
            break
            i = 0
            k = k + 1
        ...
        i = i + 1
        wl, x, y, u, v, t, h = tuple(cphotonData)
        dd = 1.0 - u ** 2 - v ** 2
        w = sqrt(dd) if dd > 0. else 1.0
