...
DOCKER_CMD=run 
DOCKER_OPTS="-t --rm" 
DOCKER_VOL_IN="-v $(pwd)/io:/c764.io" 
DOCKER_VOL_OUT_TPL="-v @OUT_FOLDER@/@RUNNR@:/c764.data" 
DOCKER_IMG=c764:0.1
...
while [ $total -lt $TOTAL ]; do
    ...
    sed -e "s#@RUNNR#$run#g" \
        -e "s#@STEP#$STEP#g" \
        -e "s#@PRIM#$PRIMARY#g" \
        -e "s#@ENERGY#$ENERGY#g" ./io/in.764.tpl > ./io/in.764.rnd

    for i in 11 12 21 22 31 32 ; do 
        cat io/in.764.rnd |sed -e "s#@RND$i#$RANDOM#" > io/out.764.rnd
        mv io/out.764.rnd io/in.764.rnd
    done
    mv io/in.764.rnd  io/in
