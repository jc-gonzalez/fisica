    ...
    **/
    ...
    case 1:         // FLAG_START_OF_EVENT
      ...
      */
      ...
      for ( i=0 ; (i<ct_NPixels) && (trigger==FALSE) ; ++i ) {
        ...
        if ( ct_Type == 0 ) {
          ...
          triggerBits = pixel_bit_mask(i, q0);
          /*
          for ( int p=0 ; p<npixneig[i] && pixneig[i][p]>-1; ++p )
            cerr << ' ' << pixneig[i][p] << ':' << fnpix[pixneig[i][p]];
          cerr << "  -> " << triggerBits <<endl << flush;
          
          cout <<"TRIGGER bits::  " << triggerBits << endl << flush;
          */
          for ( j=0; j<trigger_npatterns; j++ ) {
            ...
            /*
            cerr << "comparing " << triggerBits
                 << "  and  " << trigger_patterns[j] << endl << flush;
            */
            if ( (triggerBits & trigger_patterns[j]) == trigger_patterns[j]) {
              /*
              cerr << "triggerBits=" << triggerBits
                   << "  COINCIDES with trigger_patterns[" << j
                   << "]=" << trigger_patterns[j] << endl << flush;
              */
              trigger = TRUE;
      ...
      if ( trigger == TRUE ) {
        ...
        image_data[i++] = 0.;                           // twidth
        image_data[i++] = 0.;                           // tpeak
...
create_pixels(void)
{
  ...
  float pki[ ct_NPixels ];
  float pkj[ ct_NPixels ];
  float pkk[ ct_NPixels ];
  float px [ ct_NPixels ];
  float py [ ct_NPixels ];
...
// @name add_phe_signal
//
// @desc adds single ph.e. signal to time histograms
//
...
//{
static float
add_phe_signal (float *sX, float *sY, float sigma, float amplitude_factor)
{
  static bool first = true;
  static const int number_of_sigmas = 4;
  static float *x;
  static float *y;
  static int nbins;
  static float *xtot;
  ...
  // get amplitude
  amplitude = random_von_neumann( sX, sY, nSinglePheSpectrum,
                                  maxY, aX, bX );
...
// @var y      x coordinates of the histogram (frequency)
