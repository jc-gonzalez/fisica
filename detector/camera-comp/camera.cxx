...
main(int argc, char **argv)
{
  ...
  **/
  ...
  for (i=0; i<ct_NPixels; ++i) {
    ...
    htime[i]  = new Histogram<float>(htimename, iTime_pnbins,
  ...
  for (i=0; i<ct_NPixels; ++i) {
    ...
    hpulse[i] = new Histogram<float>(htimename, iTime_pnbins,
             fTime_pmintime, fTime_pmaxtime);
  }
  ...
  for (i=0; i<ct_NPixels; ++i) {
    sprintf(htimename,"Time response for pixel #%d", i+1);
    htrigger[i] = new Histogram<float>(htimename, iTime_discnbins,
  ...
  Histogram<float> hcoincidences("Coincidences histogram", iTime_discnbins,
         fTime_discmintime, fTime_discmaxtime);

  // histogram with widths of distributions
  Histogram<float> htimewidth("Width of Time distributions",
            1000, 0.0, fTime_maxtime-fTime_mintime);
  ...
  Histogram<float> pmtpulse("PMT response", 200, 0.0, fTime_maxtime);
    ...
    case 1:         // FLAG_START_OF_EVENT
      ...
      **/
      ...
      for ( i=0 ; (i<ct_NPixels_small) && (trigger==FALSE) ; ++i ) {
        ...
        for ( ntimebin=0 ;
              (ntimebin<iTime_discnbins) && (trigger==FALSE) ;
              ++ntimebin) {
      ...
      if ( trigger == TRUE ) {
        ...
        // fill data array with OVERALL sum of signal
        for ( i=0; i<ct_NPixels; ++i ) {
          fnpixclean[i] = 0.0;
          for ( j=0; j<iTime_pnbins; ++j )
