...
c @code
      ...
      SUBROUTINE attenu(wavelength, height, theta, tr_atmos, m1, m2, m3)
...
c--
      ma = (1.D0/H)*(SQRT((RT*COS(theta))**2+(2*RT*H)+H**2)-
     +     (RT*COS(theta)))
c--
      mb = (1.d0/(h-obslev)) * 
     &     ( -(rt+obslev)*cos(theta) 
...
c--
      ...
      x1 = sqrt((2. * obslev + Rcos2) / (2. * 7.4e5))
      x2 = sqrt((2. * h      + Rcos2) / (2. * 7.4e5))
      x3 = sqrt((2. * obslev + rt)    / (2. * 7.4e5))
      x4 = sqrt((2. * h      + rt)    / (2. * 7.4e5))
      ...
      m = exp(-Rsin2 / (2. * 7.4e5)) * ((e1 - e2) / (e3 - e4))
