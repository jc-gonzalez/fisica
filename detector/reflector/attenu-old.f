...
c// @text 
c// This code is Copyright $\copyright$ 1998, Aitor Ibarra Ibaibarriaga
c// @endtext
c @code
...
C-----------------------------------------------------------------------
      ...
      DOUBLE PRECISION AATM(5),BATM(5),CATM(5),LAHG(5),RHOF(5)
...
c      IF     ( H .GT. 4.D5 ) THEN
c         I = 2
c      ELSEIF ( H .GT. 1.D6 ) THEN
c         RHOF(2) = BATM(2)*(EXP ( -(4.D5/CATM(2))) + 
c     +        EXP(-(1.D6/CATM(2))))
c         I = 3
c      ELSEIF ( H .GT. 4.D6 ) THEN
c         RHOF(3) = BATM(3)*(EXP ( -(1.D6/CATM(3))) + 
c     +        EXP(-(4.D6/CATM(3))))
c         I = 4
c      ELSEIF ( H .GT. 1.D7 ) THEN
c         RHOF(4) = BATM(4)*(EXP ( -(4.D6/CATM(3))) + 
c     +        EXP(-(1.D7/CATM(3))))
c         RHOF(5) = CATM(5)*(H-1.D7)
c         I = 5
c      ELSE
c         RHOFP = BATM(I)*(EXP ( -(H/CATM(I))) + 
c     +        EXP(-(LAHG(I)/CATM(I))))
c      ENDIF
c   
c      
c      DO 10 J=1,(I-1)
c        RHO_TOT = RHO_TOT + RHOF(I)      
c 10   CONTINUE
c      
c      RHO_FI = m*(RHO_TOT + RHOFP)
...
***************************************************************************
      ...
      IF (H.LT.10.0D5) THEN
         ...
         T_Mie = EXP ((Hm/Lm)*m*(EXP(-H/Hm)-EXP(-2200.D2/Hm)))
...
*     TOTAL TRANSMISSION OF THE ATMOSPHERE: (RAYLEIGH + MIE + OZONE)
 ...
 9999 tr_atmos = T_Ray*T_Mie*T_Oz
c      trr = T_Ray
c      trm = T_Mie
c      tro = T_Oz
