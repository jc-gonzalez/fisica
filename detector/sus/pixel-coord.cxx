...
@section Comments on {\tt pixel-coord.cxx}

@text

In this section we show the (commented) code of the program for the
pre-calculation of the coordinates of the pixels in the camera, in a
tri-axis hexagonal frame, defined below. This definition of hexagonal
coordinates is taken from Charles Fu, who proposed these coordinates
long ago for hexagonal grids on the plane. Thanks.

@endtext

@subsection Definition of the tri-axis hexagonal frame

@text

In the figure \ref{fig:pixels} is shown the definition of the tri-axis
frame used for the numbering of the pixels. The unit in each of the
three axes is $1.5\times$Corner-to-Corner/2 of a pixel with
edge-to-edge width of 1.0. With this definition the coordinates of the
pixels are, for example for the first seven pixels: 1:(0,0,0),
2:(1,-1,0), 3:(1,0,-1), 4:(0,1,-1), 5:(-1,1,0), 6:(-1,0,1),
7:(0,-1,1).
...
// @subsection Includes and macro definitions
