...
## fuzzy
##
## @file        fuzzy.m @title       Studies on fuzzy logic for
## $\gamma$-h separation @subtitle    Use of Adaptive Fuzzy Systems
## @author J C Gonz\'alez @email       gonzalez@mppmu.mpg.de
##
## @maintitle
######################################################################
...
for k=1:max(size(fractions)),
  ...
  for t=1:ntimes;
    ...
    q(kk) = (nn(3)/ngamm) / sqrt( nn(1)/nhadr );

    if (shw==1)
      printf("%10.6f %10d %10d %10d %10d %10d %10d %f\n", 
	     fr, npts, ngamm, nhadr, nn(1), nn(2), nn(3), q(kk));
    end
